...
const squareCode = function(message) {
  let msgStr = message.split(' ').join('');
  let msgLen= msgStr.length;
  let root = Math.ceil(Math.sqrt(msgLen));
  let square = [];
  let tempPos = 0;
  let pos = 0;
  let resultStr = '';
  ...
  for(let i = 0; i <= root;i++){
    if(tempPos < msgLen){
      square[i] = msgStr.substr(tempPos,root);
      tempPos+=root;
    }
  }
